...
The demo client itself is in development stage. In "good and normal weather", the `Thingy:91` runs from battery for 7-15 months, exchanging every hour a message.
...
For `nRF9161` and `nRF9151` devices use the modem firmware [2.0.4](https://nsscprodmedia.blob.core.windows.net/prod/software-and-other-downloads/sip/nrf91x1-sip/nrf91x1-lte-modem-firmware/mfw_nrf91x1_2.0.4.zip) for that. 
...
| [**EXT** Conexio Stratus Pro nRF9161](https://conexiotech.com/conexio-stratus-pro-nrf9161/)<br>Requires additional batteries, antennas, and closures to work in the "wild". Comes with built-in solar energy harvesting, motion sensor, 2KB EEPROM, and 2x5 plug for JTAG. Includes a SIM card with 500MB in 10 years. | [<img src="https://conexiotech.com/wp-content/uploads/2024/02/D-copy.png" width="300"/>](https://conexiotech.com/wp-content/uploads/2024/02/D-copy.png) |

**Note:** for all supported **EXT** boards you need to copy the board definitions in folder `extra` into the destination folders. The manufacturer of the boards already announced to offer support for NCS 2.7.0 and newer, so the full support will only come with NCS 2.9.2. 
...
## Required HW-Tools for nRF9160 feather v5 (or newer)
...
- a debug probe to flash the device.
   ...
   - or a [nRF5340-DK (doesn't work for the `Thingy:91`!)](https://www.nordicsemi.com/Products/Development-hardware/nrf5340-dk)
...
## Required HW-Tools for nRF9151 feather 

In order to use this demo with a `nRF9151 feather`, you need:

- a `nRF9151 feather` (maybe better two ;-)).
- the `nRF9151 feather` is shipped with a SIM card. Check, if that covers your [area/country](https://www.hologram.io/coverage/). If not, you need a SIM card for your area/country.
- the `nRF9151 feather` comes with a internal debug probe to flash the device. For flashing no additional equipment is required.
- to debug the device you will need a [Recovery & Programming Mini Fixture](https://www.circuitdojo.com/products/nrf9151-feather) and
   - either a [nRF9160-DK](https://www.nordicsemi.com/Products/Development-hardware/nRF9160-DK) or a [nRF5340-DK (doesn't work for the `Thingy:91`!)](https://www.nordicsemi.com/Products/Development-hardware/nrf5340-dk) and a `10-wire-ribbon connector, 2x5, 0.050"` from [Harwin Inc.](https://www.digikey.com/en/products/detail/harwin-inc/M50-9100542/4953091) or [Adafruit Industries LLC](https://www.digikey.de/de/products/detail/adafruit-industries-llc/1675/6827142).
   - or a [Segger j-Link](https://www.segger.com/products/debug-probes/j-link/) and a [cortex-M adapter](https://www.segger.com/products/debug-probes/j-link/accessories/adapters/9-pin-cortex-m-adapter/).
...
In order to use this demo with a `Conexio Stratus Pro nRF9161 or nRF9151`, you need:

- a `Conexio Stratus Pro nRF9161 or nRF9151` (maybe better two ;-)).
- the `Conexio Stratus Pro` is shipped with a SIM card. Check, if that covers your [area/country](https://www.1nce.com/en-eu/1nce-connect/our-coverage). If not, you need a SIM card for your area/country.
- a debug probe to flash the device.
   - either a [nRF9160-DK](https://www.nordicsemi.com/Products/Development-hardware/nRF9160-DK) and a `10-wire-ribbon connector, 2x5, 0.050"` from [Harwin Inc.](https://www.digikey.com/en/products/detail/harwin-inc/M50-9100542/4953091) or [Adafruit Industries LLC](https://www.digikey.de/de/products/detail/adafruit-industries-llc/1675/6827142).
   - or a [Segger j-Link](https://www.segger.com/products/debug-probes/j-link/) and a [cortex-M adapter](https://www.segger.com/products/debug-probes/j-link/accessories/adapters/9-pin-cortex-m-adapter/).
