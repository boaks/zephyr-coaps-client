...
#define SETTINGS_KEY_CALIBRATION_A "cal_a"
#define SETTINGS_KEY_CALIBRATION_B "cal_b"
...
static size_t cal_a_length = 0;
static size_t cal_b_length = 0;
static uint8_t cal_a[MAX_SETTINGS_VALUE_LENGTH] = {0};
static uint8_t cal_b[MAX_SETTINGS_VALUE_LENGTH] = {0};
#endif
                                    ...
                                    void *cb_arg)
{
   ...
   if (!next) {
      ...
      if (appl_settings_key_match(name, SETTINGS_KEY_CALIBRATION_A, name_len)) {
#ifdef CONFIG_NAU7802_SCALE
         res = read_cb(cb_arg, &buf, sizeof(cal_a));
         k_mutex_lock(&settings_mutex, K_FOREVER);
         memcpy(cal_a, buf, sizeof(cal_a));
         cal_a_length = res > 0 ? res : 0;
         k_mutex_unlock(&settings_mutex);
         if (!res) {
            LOG_INF("calibration A: n.a.");
         } else if (res > 0) {
            LOG_INF("calibration A: %d bytes", res);
         }
#endif /* CONFIG_NAU7802_SCALE */
         return 0;
      }

      if (appl_settings_key_match(name, SETTINGS_KEY_CALIBRATION_B, name_len)) {
#ifdef CONFIG_NAU7802_SCALE
         res = read_cb(cb_arg, &buf, sizeof(cal_b));
         k_mutex_lock(&settings_mutex, K_FOREVER);
         memcpy(cal_b, buf, sizeof(cal_b));
         cal_b_length = res > 0 ? res : 0;
         k_mutex_unlock(&settings_mutex);
         if (!res) {
            LOG_INF("calibration B: n.a.");
         } else if (res > 0) {
            LOG_INF("calibration B: %d bytes", res);
         }
#endif /* CONFIG_NAU7802_SCALE */
         return 0;
      }
...
#ifdef CONFIG_NAU7802_SCALE
   if (cal_a_length) {
      (void)cb(SETTINGS_SERVICE_NAME "/" SETTINGS_KEY_CALIBRATION_A, cal_a, cal_a_length);
   }
   if (cal_b_length) {
      (void)cb(SETTINGS_SERVICE_NAME "/" SETTINGS_KEY_CALIBRATION_B, cal_b, cal_b_length);
   }
#endif
...
int appl_settings_get_bytes(const char *id, int64_t *time, uint8_t *data, size_t data_size)
{
   ...
   k_mutex_lock(&settings_mutex, K_FOREVER);
   if (!strcmp(id, "CHA")) {
      memmove(&buf, cal_a, MAX_SETTINGS_VALUE_LENGTH);
      res = cal_a_length;
   } else if (!strcmp(id, "CHB")) {
      memmove(&buf, cal_b, MAX_SETTINGS_VALUE_LENGTH);
      res = cal_b_length;
   }
   k_mutex_unlock(&settings_mutex);
...
int appl_settings_set_bytes(const char *id, uint8_t *data, size_t data_size)
{
   ...
   k_mutex_lock(&settings_mutex, K_FOREVER);
   if (!strcmp(id, "CHA")) {
      memmove(&cal_a, buf, res);
      cal_a_length = res;
      key = SETTINGS_SERVICE_NAME "/" SETTINGS_KEY_CALIBRATION_A;
   } else if (!strcmp(id, "CHB")) {
      memmove(&cal_b, buf, res);
      cal_b_length = res;
      key = SETTINGS_SERVICE_NAME "/" SETTINGS_KEY_CALIBRATION_B;
   }
   k_mutex_unlock(&settings_mutex);
