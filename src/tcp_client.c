...
#define HTTP_HOSTNAME CONFIG_COAP_SERVER_HOSTNAME
#else
#define HTTP_HOSTNAME CONFIG_COAP_SERVER_ADDRESS_STATIC
#endif
...
int tls_cert_provision(void)
{
   ...
   if (err) {
      LOG_ERR("Failed to check for certificates err %d (%s)\n", err, strerror(-err));
      return err;
   }
...
static int tls_setup(int fd)
{
   ...
   err = setsockopt(fd, SOL_TLS, TLS_HOSTNAME, HTTP_HOSTNAME, strlen(HTTP_HOSTNAME));
   if (err < 0) {
...
int http_head(session_t *dst, int flags, uint8_t *buffer, size_t len)
{
   const char *protocol = (flags & FLAG_TLS) ? "https" : "http";
   const unsigned int port = ntohs(dst->addr.sin.sin_port);
   int header = snprintf(buffer, len, HTTP_HEAD, port);
   int result = 0;
   char *p;

   if (fd < 0) {
      result = tcp_open((flags & FLAG_TLS));
      if (result < 0) {
         goto clean_up;
      }
   }
...
int coap_post(session_t *dst, int flags, unsigned long *connected_time, uint8_t *buffer, size_t len)
{
   const char *protocol = (flags & FLAG_TLS) ? "coaps+tcp" : "coap+tcp";
   const unsigned int port = ntohs(dst->addr.sin.sin_port);
