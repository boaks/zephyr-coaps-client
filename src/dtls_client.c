...
static void dtls_trigger(void)
{
   ...
   if (dtls_no_pending_request()) {
      ...
      LOG_INF("power manager status");
      power_manager_status(NULL, NULL, NULL, NULL);
      LOG_INF("power manager status, done");
      k_sem_give(&dtls_trigger_msg);
   } else {
      LOG_INF("request pending ...");
   }
}
...
#endif
   app->provisioning = dtls_is_provisioning();
...
static int init_destination(session_t *destination, int protocol)
{
   ...
   app_data.host = dtls_get_destination();

   if (strlen(app_data.host) > 0) {
      int count = 0;
   ...
   destination->addr.sin.sin_port = htons(dtls_get_destination_port(!(protocol & 1)));
...
//   accelerometer_enable(false);
   ...
   if (protocol == 0) {
      dtls_init();
      dtls_settings_init(imei, &cb);
   } else {
      dtls_settings_init(imei, NULL);
   }
   coap_client_init();
