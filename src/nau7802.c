...
static int scale_read_channel_value(struct scale_config *scale_dev, int max_loops, int min_values, int32_t max_dither)
{
   ...
   scale_dev->err_dither = 0;
   scale_dev->err_max_dither = 0;
   values[counter++] = v;
   max = v;
   min = v;
   do {
      ...
      delta = max - min;
      if (delta > max_dither) {
         if (scale_dev->err_dither == 0 || scale_dev->err_dither > delta) {
            scale_dev->err_dither = delta;
         }
         if (loops < max_loops) {
   ...
   if (rc < 0) {
      ...
      scale_dev->err_max_dither = max_dither;
      rc = -ESTALE;
   } else {
      scale_dev->raw = v;
...
static int scale_read_internal_offset(struct scale_config *scale_dev)
{
   ...
   rc = scale_read_channel_value(scale_dev, MAX_INTERNAL_LOOPS, MIN_INTERNAL_ADC_SAMPLES, scale_dev->setup.max_dither - 1);
   if (rc) {
...
int scale_sample_desc(char *buf, size_t len)
{
   ...
   start = index;
   if (0 == (res & 1)) {
      if (configs[0]->err_max_dither) {
         index += snprintf(buf + index, len - index, "CHA dit-err %u < %u",
                           configs[0]->err_max_dither, configs[0]->err_dither);
      }
   }
#ifdef HAS_SCALE_B
   if (0 == (res & 2)) {
      if (configs[1]->err_max_dither) {
         if (index > start) {
            index += snprintf(buf + index, len - index, ", ");
         }
         index += snprintf(buf + index, len - index, "CHB dit-err %u < %u",
                           configs[1]->err_max_dither, configs[1]->err_dither);
      }
   }
#endif /* HAS_SCALE_B */
   if (index > start) {
