...
#ifdef CONFIG_USE_APPL_STORAGE
#include "appl_storage.h"
#include "appl_storage_config.h"
#else /* CONFIG_USE_APPL_STORAGE */
#define CALIBRATE_VALUE_SIZE 26
#endif /* CONFIG_USE_APPL_STORAGE */
...
enum dither_error {
   DITHER_NONE,
   DITHER_ERROR,
   DITHER_INTERNAL_OFFSET,
   DITHER_WEIGHT,
   DITHER_TEMPERATURE,
};

const char *DITHER_DESCRIPTION[] = {"ok", "error", "internal offset", "weight", "temperature"};
...
struct scale_config {
   ...
   enum dither_error dither_status;
   uint16_t cur_dither;
   uint16_t internal_offset_dither;
   uint16_t weight_dither;
...
static struct scale_config config_a =
    {
        ...
        .source = AVDD_UNKNOWN,
        .dither_status = DITHER_NONE,
        .cur_dither = 0,
        .internal_offset_dither = 0,
        .weight_dither = 0,
...
static inline uint8_t scale_sps_reg(uint16_t sps)
{
   if (sps < 15) {
      return 0;
   } else if (sps < 30) {
...
static inline uint8_t scale_sps_interval_ms(uint16_t sps)
{
   if (sps < 15) {
      return 100;
   } else if (sps < 30) {
...
static void scale_dump_calibration(struct scale_config *scale_dev)
{
   ...
   LOG_INF("ADC %s sps          %7u", scale_dev->channel_name, setup->sps);
...
static void scale_save_setup(struct scale_config *scale_dev)
{
   ...
   *cal++ = scale_dev->setup.pga_cap ? 1 : 0;
   sys_put_be16(scale_dev->setup.sps, cal);
   cal += 2;
   if (scale_dev->storage_config) {
#ifdef CONFIG_USE_APPL_STORAGE
...
static int scale_set_samples_per_second(struct scale_config *scale_dev, uint16_t sps)
{
   if (!scale_dev->i2c_ok) {
      return -EINVAL;
   }

   uint8_t sps_reg = scale_sps_reg(sps);
   int rc = i2c_reg_update_byte(scale_dev->i2c_device, NAU7802_ADDR, NAU7802_CTRL2, 7 << 4, sps_reg << 4);
   if (rc) {
      LOG_INF("ADC %s set samples per second %u failed,  %d (%s).", scale_dev->channel_name, sps, rc, strerror(-rc));
   } else {
      LOG_INF("ADC %s set samples per second %u", scale_dev->channel_name, sps);
      scale_dev->sample_interval_ms = scale_sps_interval_ms(sps);
   }
...
static int scale_set_pga_burnout(struct scale_config *scale_dev, bool enable)
{
   /* Enable/disable burnout */
   int rc = i2c_reg_update_byte(scale_dev->i2c_device, NAU7802_ADDR, NAU7802_I2C, BIT(2), enable ? BIT(2) : 0);
   if (rc) {
      LOG_WRN("ADC %s I2C NAU7802 %s pga burnout, write failure, %d (%s)!", scale_dev->channel_name, enable ? "enabled" : "disable", rc, strerror(-rc));
      return rc;
   }
...
static int scale_set_pga_chopper(struct scale_config *scale_dev, bool enable)
{
   /* Enable/disable pga Chopper */
   int rc = i2c_reg_update_byte(scale_dev->i2c_device, NAU7802_ADDR, NAU7802_PGA_CTRL, BIT(0), enable ? 0 : BIT(0));
   if (rc) {
      LOG_WRN("ADC %s I2C NAU7802 %s pga chopper, write failure, %d (%s)!", scale_dev->channel_name, enable ? "enabled" : "disable", rc, strerror(-rc));
      return rc;
   }
...
static int scale_read_adc_value(struct scale_config *scale_dev, int32_t *val)
{
   int32_t v = 0;
   int rc = 0;
   ...
   if (scale_dev->last_adc_time) {
      ...
      if (time < scale_dev->sample_interval_ms) {
         k_sleep(K_MSEC(scale_dev->sample_interval_ms - time));
      }
   } else {
      k_sleep(K_MSEC(scale_dev->sample_interval_ms));
   }
...
static int scale_read_channel_value(struct scale_config *scale_dev, int max_loops, int min_values, int32_t max_dither)
{
   ...
   k_sleep(K_MSEC(400 - scale_dev->sample_interval_ms)); // +SAMPLE_TIME_MS in scale_read_adc_value

   rc = scale_read_adc_value(scale_dev, &v);
   if (rc) {
      LOG_INF("ADC %s read failed, %d (%s)", scale_dev->channel_name, rc, strerror(-rc));
      return rc;
   }
   scale_dev->cur_dither = 0;
   scale_dev->dither_status = DITHER_NONE;
   LOG_INF("ADC %s => [%d] raw 0x%06x, %d", scale_dev->channel_name, counter, v & 0xffffff, v);
   values[counter++] = v;
   max = v;
   min = v;
   do {
      ...
      delta = max - min;
      if (delta > max_dither) {
         if (scale_dev->cur_dither == 0 || scale_dev->cur_dither > delta) {
            scale_dev->cur_dither = delta;
         }
         if (loops < max_loops) {
            LOG_INF("ADC %s diff: %d > %d, loop: %d, instable => retry", scale_dev->channel_name, delta, max_dither, loops);
   ...
   if (rc < 0) {
      ...
      LOG_INF("ADC %s raw 0x%06x, %d, ++/-- %d (max %d), %d<%d/%d loops, instable",
              scale_dev->channel_name, avg & 0xffffff, avg, scale_dev->cur_dither, max_dither, maxCounter, min_values, loops);
      scale_dev->dither_status = DITHER_ERROR;
      rc = -ESTALE;
   } else {
      scale_dev->raw = v;
      LOG_INF("ADC %s raw 0x%06x, %d, +/- %d <= %d, %d/%d loops",
              scale_dev->channel_name, avg & 0xffffff, avg, delta, max_dither, counter, loops);
      scale_dev->cur_dither = delta;
      rc = 0;
   }
   return rc;
...
static int scale_read_temperature(struct scale_config *scale_dev, int max_loops, int min_values, int32_t max_dither)
{
   ...
   if (scale_dev->dither_status == DITHER_ERROR) {
      scale_dev->dither_status = DITHER_TEMPERATURE;
   }
   if (rc) {
      const char *msg = strerror(-rc);
      if (!*msg && -ESTALE == rc) {
         msg = "instable values";
      }
      LOG_INF("ADC %s read temperature failed, %d (%s).", scale_dev->channel_name, rc, msg);
      goto exit_restore;
   }
...
static int scale_read_internal_offset(struct scale_config *scale_dev)
{
   int rc = -ENODATA;
   ...
   if (scale_dev->dither_status == DITHER_ERROR) {
      scale_dev->dither_status = DITHER_INTERNAL_OFFSET;
   }
   if (rc) {
      const char *msg = strerror(-rc);
      if (!*msg && -ESTALE == rc) {
      ...
      LOG_INF("ADC %s read internal offset failed, %d (%s).", scale_dev->channel_name, rc, msg);
   } else {
      scale_dev->internal_offset += scale_dev->raw;
      LOG_INF("ADC %s internal offset %d", scale_dev->channel_name, scale_dev->raw / MIN_INTERNAL_ADC_SAMPLES);
   }
...
static int scale_resume(struct scale_config *scale_dev)
{
   ...
   /* sps */
   scale_set_samples_per_second(scale_dev, scale_dev->setup.sps);
   if (rc) {
      return rc;
   }

   /* Use 5 Ohm ESR cap, select ADC reg 0x15 */
   rc = i2c_reg_update_byte(scale_dev->i2c_device, NAU7802_ADDR, NAU7802_PGA_CTRL, BIT(6) | BIT(7), BIT(6));
   if (rc) {
   ...
   /* Enable pga chopper */
   rc = scale_set_pga_chopper(scale_dev, true);
   if (rc) {
      return rc;
   }

   /* Enable bandgap chopper */
   ...
   /* disable pga burnout */
   rc = scale_set_pga_burnout(scale_dev, false);
   if (rc) {
      return rc;
   }
...
#else
      LOG_WRN("ADC %s setup EEPROM not enabled!", scale_dev->channel_name);
      return -ENOTSUP;
#endif
   }
...
static int scale_sample_channel(struct scale_config *scale_dev)
{
   ...
   if (!rc) {
      ...
      if (CALIBRATE_ZERO == phase || CALIBRATE_CHA_10KG == phase || CALIBRATE_NONE == phase) {
         ...
         scale_dev->weight_dither = scale_dev->cur_dither;
         if (scale_dev->dither_status == DITHER_ERROR) {
            scale_dev->dither_status = DITHER_WEIGHT;
         }
         if (!rc) {
         ...
         if (-ESTALE == rc) {
            LOG_INF("ADC %s scale channel instable.", scale_dev->channel_name);
         } else if (rc) {
...
int scale_sample_desc(char *buf, size_t len)
{
   ...
   if (0 < res) {
      ...
      if (configs[0]->weight != NAU7802_NONE_ADC_VALUE) {
         ...
         start = index;
         if (configs[0]->weight != NAU7802_NONE_ADC_VALUE) {
            index += snprintf(buf + index, len - index, "CHA %d/%d/%d/%d raw %u/%u/%u dither",
                              configs[0]->weight, configs[0]->internal_offset,
                              configs[0]->setup.offset, configs[0]->setup.divider,
                              configs[0]->internal_offset_dither, configs[0]->weight_dither, configs[0]->setup.max_dither);
         ...
         if (configs[1]->weight != NAU7802_NONE_ADC_VALUE) {
            index += snprintf(buf + index, len - index, "CHB %d/%d/%d/%d raw %u/%u/%u dither",
                              configs[1]->weight, configs[1]->internal_offset,
                              configs[1]->setup.offset, configs[1]->setup.divider,
                              configs[1]->internal_offset_dither, configs[1]->weight_dither, configs[1]->setup.max_dither);
            if (SCALE_INVALID_VALUE != temperatureB) {
               index += snprintf(buf + index, len - index, ", %.1f°C", temperatureB);
            }
         }
   ...
   start = index;
   if (0 == (res & 1)) {
      if (configs[0]->dither_status != DITHER_NONE) {
         index += snprintf(buf + index, len - index, "CHA dit-err %s %u > %u",
                           DITHER_DESCRIPTION[configs[0]->dither_status],
                           configs[0]->cur_dither, configs[0]->setup.max_dither);
      }
   }
#ifdef HAS_SCALE_B
   if (0 == (res & 2)) {
      if (configs[1]->dither_status != DITHER_NONE) {
         if (index > start) {
            index += snprintf(buf + index, len - index, ", ");
         }
         index += snprintf(buf + index, len - index, "CHB dit-err %s %u > %u",
                           DITHER_DESCRIPTION[configs[1]->dither_status],
                           configs[1]->cur_dither, configs[1]->setup.max_dither);
      }
   }
#endif /* HAS_SCALE_B */
   if (index > start) {
...
static int scale_set_calibration(struct scale_config *scale_dev, const char *parameter)
{
   ...
   if (!name[0]) {
      ...
      } else if (!stricmp("sps", name)) {
         res = scale_set_calibration_value(scale_dev, &cal->sps, sizeof(cal->sps), has_value, num_value, "sps");
...
static void sh_cmd_scale_calibration_help(void)
{
   ...
   LOG_INF("           gain <value>   : set calibration gain.");
   LOG_INF("                          : 1, 2, 4, 8, 16, 32, 64, 128");
   LOG_INF("           sps <value>    : set calibration samples per second.");
   LOG_INF("                          : 10, 20, 40, 80, 320");
   ...
   LOG_INF("  scalecal [CHA|CHB] [zero|ref|<reference>]:");
   LOG_INF("           zero           : measure 0 to calibrate offset.");
   LOG_INF("           ref            : measure 10kg to calibrate divider.");
